...
/**
 * 0034: minijuegos. Ranking POR RUTA y cervezas guardadas.
 *
...
const cerveza = (db: PGlite, ruta: string, nombre = 'Mi ca̱a', receta: readonly [string, string, number, number, number] = ['palida', 'ale', 100, 100, 100]) =>
  db.query<{ r: Record<string, unknown> }>(
    `select public.maestro_submit_beer($1, $2, $3, $4, $5, $6, $7) as r`,
    [ruta, nombre, ...receta],
  );

describe('0034: las tablas no se tocan por la API', () => {
...
describe('0034: solo quien esta dentro de la ruta', () => {
...
describe('0034: el ranking de La Cana Perfecta', () => {
...
describe('0034: Maestro Cervecero, el servidor calcula la cerveza', () => {
...
describe('0034: la lista de cervezas', () => {
...
describe('0034: se va con la ruta y con la cuenta, y sale en "descargar mis datos"', () => {
  const sembrar = async (db: PGlite, a: Actor) => {
    await a.como(ANA);
    await nota(db, RUTA, 77);
    await cerveza(db, RUTA, 'Mi ca̱a');
  };
  ...
  it('export_my_data incluye los records y las cervezas, con el nombre de la ruta', async () => {
    await conBase(async (db, a) => {
      ...
      assert.equal(mj?.cervezas[0]?.nombre, 'Mi ca̱a');
