...
/**
 * 0008 anade bloquear y denunciar. Lo que hay que asegurar sobre Postgres real:
...
describe('0008_cana_bloqueos_denuncias.sql: forma', () => {
  ...
  it('las tablas nuevas no tienen privilegios para la app', () => {
    const sinComentarios = m0008.replace(/--.*$/gm, '');
    assert.match(
      sinComentarios,
...
describe('0008 sobre Postgres real', async () => {
  ...
  const db = await crearBase([m0001, m0002, m0004, m0005, m0006, m0007, m0008, m0008]);
  await db.exec(DATOS);
...
describe('0008: contrato con el panel de administracion', async () => {
  const db = await crearBase([m0001, m0002, m0004, m0005, m0006, m0007, m0008]);
  await db.exec(DATOS);
