...
/**
 * 0010: consentimiento guardado al activar, y poder llevarte o borrar tus
...
const m0010 = leerFichero('supabase/migrations/0010_cana_consentimiento_y_datos.sql');
...
describe('0010_cana_consentimiento_y_datos.sql: forma', () => {
  it('la gramatica es valida y los cuerpos plpgsql compilan', async () => {
    const resultado = (await init()).parse(m0010);
    ...
    const plpgsql = (await init()).parsePlpgsql(m0010);
    assert.ok(!plpgsql.error, `plpgsql no compila: ${JSON.stringify(plpgsql.error)}`);
  });
});

describe('0010 sobre Postgres real', async () => {
  // La 0010 dos veces: tiene que poder re-ejecutarse como las demas.
  const db = await crearBase([m0001, m0002, m0005, m0006, m0007, m0008, m0009, m0010, m0010]);
  await db.exec(DATOS);
