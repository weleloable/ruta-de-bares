/**
 * Traduce los errores de las funciones de la 0030 a algo que se pueda leer en
...
export function traducirErrorMinijuegos(mensaje: string): string {
  ...
  // PostgREST cuando la funcion no existe: la migracion 0030 no esta pegada.
  if (/PGRST202|Could not find the function|schema cache/i.test(mensaje)) {
    return 'Falta aplicar la migración 0030 en Supabase (ver docs/SETUP.md).';
  }
